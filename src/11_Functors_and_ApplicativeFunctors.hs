...
--- ¶ Zip リスト
{-
    実は、リストをアプリカティブファンクターにする方法は複数ある。1 つ目の方法はもう学んだとおりで、<*> を関数のリストと値のリストに対して呼び出して、
    左辺のリストからの関数と右辺のリストからの値のあらゆる可能な組み合わせを作り、適用したリストを返すというもの。

    たとえば、[(+3), (*2)] <*> [1,2] というふうに書いたら、(+3) は 1 と 2 の両方に適用され、
    (*2) も 1 と 2 の両方に適用されるので、結果として 4 つの要素からなるリスト [4,5,2,4] ができる。
    ところが、[(+3), (*2)] <*> [1,2] の挙動は、左辺の 1 つ目の関数を右辺の 1 つ目の値に適用し、左辺の 2 つ目の関数を右辺の 2 つ目の値に適用する、というのではまずいのだろうか？
    それなら結果は [4,4] になるはずである。これは [1 + 3, 2 * 2] と考えることもできる。

    ここで初めて登場する Applicative のインスタンスが ZipList である（Control.Applicative にある）。
    1 つの型に対して、同じ型クラスのインスタンスを複数回宣言することはできないので、[] の代わりに ZipList a 型を導入する（つまり気持ち的にはリストの亜種みたいなもの）。
    これは、1 つのコンストラクタ（ZipList）と、ただ 1 つのフィールド（a のリスト）を持つ。以下の型宣言を参照。
        ZipList :: [a] -> ZipList a

    それで、以下が Applicative 型クラスのインスタンスですよというインスタンス宣言。
        instance Applicative ZipList where
            pure x = ZipList (repeat x)
            ZipList fs <*> ZipList xs = ZipList (ZipWith (\f x -> f x) fs xs)

    <*> は、1 つ目の関数を 1 つ目の値に、2 つ目の関数を 2 つ目の値に、…… と適用する。これをやっているのが ZipWith (\f x -> f x) fs xs である。
    zipWith の仕様上、出来上がるリストは 2 つのリストのうち短い方の長さになる。

        >>> 復習：ちなみに zipWith の使い方は。。。
        > zipWith (+) [1,2,3] [10, 20, 30]
        [11,22,33]

    pure の定義も面白い。ZipList の pure は、引数を取って、それを永遠に繰り返すリストに突っ込む。
    例えば pure "nya" は ZipList (["nya", "nya", "nya", ... を返す。これはおかしいと思うかもしれない。
    pure は引数を再現できる最小の文脈に入れるものなのに、無限リストはとても最小の文脈に見えない。
    しかし ZipList に限っては、pure はリストのあらゆる一で値を再現してほしいので、この定義でよいのである。
    それにこの定義なら、pure f <*> xs と fmap f xs は等価であれ、という法則を満たす。
    もし pure 3 がただの ZipList [3] しか返さない仕様だったら、pure (*2) <*> zipList [1,5,10] の結果は zipList [2] になってしまう。
    2 つのリストと zip した結果の長さは短いほうの長さになるあkらである。
    一方、無限リストと有限リストを zip した結果のリストの長さは、有限リストのほうの長さになる。

    さて、ZipList をアプリカティブ・スタイルで試してみよう！
    と言いたいところだが、ZipList a 型は Show インスタンスをサポートしていない。getZipList 関数を使って生リストを取り出す必要がある。
-}

zipList1 :: [Int]
zipList1 = getZipList $ (+) <$> ZipList [1,2,3] <*> ZipList [100, 100, 100] -- [101,102,103]

zipList2 :: [Int]
zipList2 = getZipList $ (+) <$> ZipList [1,2,3] <*> ZipList [100, 100..] -- [101,102,103]

zipList3 :: [Int]
zipList3 = getZipList $ max <$> ZipList [1,2,3,4,5,3] <*> ZipList [5,3,1,2] -- [5,3,3,4]

zipList4 :: [(Char, Char, Char)]
zipList4 = getZipList $ (,,) <$> ZipList "dog" <*> ZipList "cat" <*> ZipList "rat" -- [('d','c','r'),('o','a','a'),('g','t','t')]
    -- 関数 (,,) は \x y z -> (x, y, z) と同じ意味。同様に、(,) と書いたら、\x y -> (x, y) という意味。
    {-
    *Main Control.Applicative> (,) 1 2
    (1,2)
    *Main Control.Applicative> (,,) 10 20 30
    (10,20,30)
    -}

-- pure の使い方
zipList5 :: [Int]
zipList5 = getZipList $ (+) <$> pure 5 <*> ZipList [10, 20, 30] -- [15,25,35]

-- ZipList における pure が無限リストを生んでいることを確かめたければターミナルで getZipList $ pure 5 というように入力すれば出力が [5,5,5,5,5,5,5,5,...] というふうになる。

{-
    zipWith 以外にも、標準ライブラリは zipWith3、zipWith4、...、zipWith7 までの関数を備えている。
    zipWith は 2 引数関数を取って、2 つのリストをそれで綴じ合わせる。
    zipWith3 は 3 引数関数を取って、3 つのリストを綴じ合わせる。以下同様。

    ところが、zipList をアプリカティブ・スタイルで使えば、閉じ合わせたいリストの数ごとに zipWith 関数を使い分ける必要はなくなる。
    アプリカティブ・スタイルを使えば、好きな数のリストを 1 つの関数で綴じ合わせることができるのである。
-}

--- ¶　アプリカティブ則
{-
    普通のファンクターのように、アプリカティブファンクターにもいくつかの法則が付いてくる。
    中でも一番重要なのは pure f <*> x = fmap f x である。
    これを含めたアプリカティブ則の一覧は以下のとおり。
    ...
    （ほんの少しの例）
        ...
        *Main Control.Applicative> pure (+10) <*> pure 2
        12
-}
...
{-
    ...
    liftA2 は、これまで見てきたアプリカティブ・スタイルを省略しつつ、1 つの関数を 2 つのアプリカティブ値に適用するだけの関数である。
    しかし、アプリカティブファンクターが通常のファンクターに勝っている点を明瞭に示す例になっている。

    通常のファンクターでは、関数を 1 つのファンクター値に適用することしかできない。
    アプリカティブファンクターなら、関数をいくつものファンクター値に適用できる。
    liftA2 の型を (a -> b -> c) -> (f a -> f b -> f c) と解釈すると、これがまた面白い。
    この見方をすれば、liftA2 は「通常の 2 引数関数を、2 つのアプリカティブ値を引数に取る関数に昇格させる」関数だとみなせる。
    （🤔だから lift って名前なのだろう。関数の持ち上げ（lifting）については本章でもすでに出てきた（ファンクターにおける fmap の説明にて）
    　そのとき、関数を持ち上げるとされた fmap も型は fmap :: (Functor f) => (a -> b) -> f a -> f b で、まさに同様の形）

    ここで 1 つ、面白い概念を紹介しよう。2 つのアプリカティブ値から、それらの返り値をリストとして内包する 1 つのアプリカティブ値を組み立てることができる。
    例えば Just 3 と Just 4 があるとする。まずは 2 つ目の中身の 4 を単一要素リストに入れよう。
    これはとても簡単。
-}

happy :: Maybe [Int]
happy = fmap (\x -> [x]) (Just 4) -- Just [4] // 💥[Just 4] ではないので注意！ fmap (*2) Just 4 だったら Just 8 になるのと同じ。中身に関数が適用される。

{-
    OK。いま手札には Just 3 と Just [4] があることになった。ここから Just [3,4] を作るにはどうしたらいいか？　これも簡単。
-}

happy' :: Maybe [Int]
happy' = liftA2 (:) (Just 3) (Just [4]) -- Just [3,4]

{-
    : は、ある要素とあるリストを取って、その要素を先頭に付けた新しいリストを作る関数だった。
    （普段は 1:[] のように中置関数として使っているだけで、(:) 1 [] としても結果は [1] になる）
    さて、Just [3,4] はできた。次にこれを Just 2 と組み合わせて Just [2,3,4] を作ることはできるだろうか？　もちろんできる。
    どうやら、好きな数のアプリカティブ値たち（Just 3, Just 4 など）から、それらの返り値をリストにしたもの（= [3,4]）を持つ単一のアプリカティブ値（= Just [3,4]）を組み立てることは常に可能なようである。

    では、「アプリカティブ値のリスト」（= [Just 3, Just 4] など）を取って、「リストを返り値として持つ 1 つのアプリカティブ値」（= Just [3,4] など）を返す関数を実装してみよう。
    これを sequenceA' と呼ぶことにする。（以下で実装するけど、実は Prelude に sequenceA は組み込まれている！）
-}

-- sequenceA は実は Prelude に存在するので、' をつける。
sequenceA' :: (Applicative f) => [f a] -> f [a]
sequenceA' [] = pure []
sequenceA' (x:xs) = (:) <$> x <*> sequenceA' xs

{-
    再帰である！　まずは型を見てみよう。この関数は、アプリカティブ値のリストを、リストを持っているアプリカティブ値に変える。
    この方からだけでも、再帰の基底部を書くことができる。空リストをリストを持っているアプリカティブ値にしたかったら、単にデフォルトの文脈に入れるだけである。
    次は再帰部。リストの ehad と tail が与えられたなら（x は単一のアプリカティブ値で、xs はアプリカティブ値のリスト）、まず sequenceA' を tail に対して呼んで
    リストの入ったアプリカティブ値を返してもらい、pure (:) を使ってアプリカティブ値 x の中身をそのリストに結合すればよい。

    使用例は以下。
-}

hoppy :: Maybe [Int]
hoppy = sequenceA' [Just 1, Just 2] -- Just [1,2]
{-
    定義より、これは以下のように書くのと同値。
    (:) <$> Just 1 <*> sequenceA' [Just 2]

    さらに評価を進める（再帰をたどる）と、以下のようになる。
    (:) <$> Just 1 <*> ((:) <$> Just 2 <*> sequenceA' [])

    ここで、sequenceA' [] は pure [] つまりここでは Just [] になるので
    (:) <$> Just 1 <*> ((:) <$> Just 2 <*> Just [])
...
{-
    sequenceA は、畳み込みを使っても実装できる。リストの要素を走査しながら何らかの結果を集計するという操作は、
    たいてい畳み込みを使って実装できるということを覚えておこう。
-}
...
{-
    リストを右から処理し、アキュムレータの初期値は pure [] にしておく。
    次に liftA2 (:) がアキュムレータとリストの最後の要素に適用され、単一要素リストが入ったアプリカティブ値ができる。
    続いて liftA2 (:) が今の最終要素（元リストの最後から 2 番目）とアキュムレータに適用され、
    2 要素リストが入ったアプリカティブ値ができ、……同様に続けて、最後にすべてのアプリカティブの返り値のリストが入っているアキュムレータだけが残る。
        🐱復習 〜foldr の使い方〜 🐱
        > foldr  (++) "0" ["1","2","3"]
        "1230"
    さっそく、この関数をアプリカティブ値に対して使ってみよう。
-}

yummy :: Maybe [Int]
yummy = sequenceA'' [Just 3, Just 2, Just 1] -- Just [3,2,1]

moon :: Maybe [Int]
moon = sequenceA'' [Just 3, Nothing, Just 1] -- Nothing

night :: Num a => a -> [a]
night = sequenceA'' [(+3), (+2), (+1)]

night10 :: [Int]
night10 = night 10 -- [13,12,11]

listoflist :: [[Int]]
listoflist = sequenceA'' [[1,2,3], [4,5,6]] -- [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6],[3,4],[3,5],[3,6]]

listoflistWithEmpty :: [[Int]]
listoflistWithEmpty = sequenceA'' [[1,2,3],[4,5,6],[7,8,9],[]] -- []

{-
    Maybe 値のリストに対して sequenceA を使うと、すべての返り値をリストとして含んだ単一の Maybe 値ができる
    （返り値という言い方がいまいちピンと来ないんだけど、sequence A [Just 3, Just 2, Just 1] からは Just [3,2,1] という単一の Maybe 値ができるということ）。
    ただし、元のリストの中の Maybe 値のいずれかが Nothing であった場合、結果も Nothing になる。
    この振る舞いは、Maybe 値のリストがあり、どれも Nothing でない場合に限って中身に興味がある、という場合に便利。

    関数に対して sequenceA を使うと、sequenceA は「関数のリスト」を取って（=上の例では `night = sequenceA [(+3),(+2),(+1)]`）、
    「リストを返す関数」を返す（=night に 10 を引数として渡すと [13, 12, 11] というリストが返ってくる）。
    上の例では、ある数を引数に取り、リストの中の各関数を適用した結果をリストとして返すような関数を作っている。
    sequenceA [(+3), (+2), (+1)] 3 は、関数 (+3) を引数 3 に、関数 (+2) を 引数 3 に、関数 (+1) を引数 3 に適用し、
    その結果（=それぞれ 6, 5, 4）を [6,5,4] というふうにリストとして返す。

    前に (+) <$> (+3) <*> (*2) と書いたときには、「1 つの引数を取って (+3) と (*2) の両方を適用し、その 2 つの結果を * する」関数が作れた。
    これと同じノリで、sequenceA [(+3), (*2)] と書くと、「1 つの引数を取ってそれをリストの中のすべての関数に食わせる」関数が作れるのである。
    ただし sequenceA では、返り値を + で結合するかわりに、: と pure [] の組み合わせによって返り値たちを 1 つのリストに結合し、
    それが sequenceA [(+3), (*2)] 全体の返り値になる。

    sequenceA は、関数のリストがあり、そのすべてに同じ引数を食わせて結果をリストとして眺めたい、という場合にはとても便利である。
    例えば、ある数がいくつかの性質をすべて満たしているかテストしたいときなどである。
    sequenceA を使わない場合では、例えば以下のようなやり方がある。
-}

tfs :: [Bool]
tfs = map (\f -> f (7::Int)) [(>4), (<10), odd] -- [True, True, True]

tfs' :: Bool
tfs' = and $ map (\f -> f (7::Int)) [(>4), (<10), odd] -- True

--- > sequenceA を使うと別の書き方ができる。
tfs_ :: [Bool]
tfs_ = sequenceA [(>4), (<10), odd] (7::Int) -- [True, True, True]

tfs_' :: Bool
tfs_' = and $ sequenceA [(>4), (<10), odd] (7::Int) -- True

{-
    sequenceA [(>4), (<10), odd] が作る関数は、引数として取った数に [(>4), (<10), odd] のそれぞれを適用し、Bool のリストを返す。
    sequenceA は、(Num a) => [a -> Bool] の型を持ったリストを (Num a) => a -> [Bool] という型の関数に変えたのである。すごい！

    リストの中の関数はすべて同じ型でないといけない。例えば、[ord, (+3)] などというリストは作れない。
    ord は文字（Char）を取って数値を返すのに対して、(+3) は数値を取って数値を返すからである。

    sequenceA を [] に対して使うと、リストのリストを取ってリストのリストを返す関数になる。
    実際の動作は、1 番目のリストから 1 つ、2 番目のリストから 1 つ、……というふうに、要素を取って作れるリストwおすべて列挙するというものになる。
    説明のために、先ほどの例を sequenceA およびリスト内包表記を使って再現してみる。
-}

-- sequenceA を使った例（`listoflist` の再掲）
listSeq :: [[Int]]
listSeq = sequenceA [[1,2,3], [4,5,6]] -- [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6],[3,4],[3,5],[3,6]]

-- リスト内包表記を使ったら
listSeq' :: [[Int]]
listSeq' = [[x,y] | x <- [1,2,3], y <- [4,5,6]] -- [[1,4],[1,5],[1,6],[2,4],[2,5],[2,6],[3,4],[3,5],[3,6]]

{-
    (+) <$> [10,20] <*> [4,5,6] と書くと、x は [10, 20] のいずれだかわからず、y は [4,5,6] のいずれだかわからないという文脈で、
    非決定性計算 x + y を行ってくれるのだった。
    そして、非決定性計算の結果は、可能な答えをすべて列挙したリストとして表現されるんだった（[14,15,16,24,25,26] になる）。
    同様に、sequence [[1,2], [3,4], [5,6]] と書くと、その結果は、x は [1,2] のいずれだかわからず、y は [3,4] のいずれかわからず、
    z は [5,6] のいずれかわからない、というときに可能な [x,y,z] の組み合わせを列挙する、という非決定性計算になる。
    可能な答えの 1 つひとつがリストであり、非決定性計算であることを表現するのに答えのリストを使うので、全体の結果は二重リストになっているわけである。
-}

{-
    I/O アクションに対して使ったときの sequenceA は、まさに sequence である。
    「I/O アクションのリスト」を引数に取って、「各 I/O アクションを実行し、返り値をリストとして返す I/O アクション」を返す関数になる。
    どうしてこうなるかというと、実行したら結果のリストを返すような I/O アクションを作るために [IO a] 型の値を IO [a] 型に変えたい、と思ったら、
    まずすべての I/O を直結し、評価が強制され次第 I/O を 1 つずつ実行するしかないからである。
    I/O アクションの結果は実行しないと取り出すことができない。
    それでは 3 つの I/O アクション getLine を結合してみよう。
-}

ioSeq :: IO [String]
ioSeq = sequenceA [getLine, getLine, getLine]

-- 使用例
-- *Main Control.Applicative> ioSeq
-- hoge
-- fuga
-- nyan
-- ["hoge","fuga","nyan"]
...
{-
    まとめると、アプリカティブファンクターは、面白い上にとても便利なものである。アプリカティブファンクターを使えば、I/O を伴う計算、
    非決定性計算（リスト）、失敗するかもしれない計算（Maybe）、……などなど、多種多様な計算をアプリカティブ・スタイルを使って組み合わせることができる。
    <$> と <*> を使うだけで、普通の関数をどんなアプリカティブファンクターとも組み合わせて使うことができ、それぞれのアプリカティブファンクターの文脈の力を借りることができるのである。
-}
