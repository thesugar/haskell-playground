...
-------------------------------
{-
    この節では、型が生まれてモナドであると確認され、適切な Monad インスタンスが与えられるまでの過程を、例題を通して学ぼう。
    普通、モナドは作りたいと思って作るものではない。むしろ、とある問題のとある側面をモデル化した型を作り、後からその型が文脈付きの値を表現していて
    モナドのように振る舞うとわかった場合に、Monad インスタンスを与える場合が多い。

    これまでにも見てきたように、リストは非決定的な値を表現している。
    例えば [3,5,9] のようなリストは、どの値を取るか決めかねている非決定的整数であると解釈できる。
    >>= を使ってリストを関数に食わせたら、リストからあらゆる可能な選択肢が取り出され、それぞれに関数が適用されて、結果がまたもリストとして提示されるわけである。

    リスト [3,5,9] を、整数 3、5、9 が同時に存在している状態だと思って眺めていると、あれ、でもそれぞれの数の存在確率の情報がなくないか？　と気づく。
    たとえば 3 である確率が 50 %、5 と 9 はそれぞれ 25 % である、ということを表現したくなったらどうする？　よし、やってみよう。

    リストの要素のそれぞれが、もう 1 つの値を伴っているとしよう。そのもう 1 つの値とは、その要素g実現する確率である。その値を以下のように表現するのはどうだろう。
        [(3, 0.5), (5, 0.25), (9, 0.25)]
    確率を浮動小数で表していたら、すぐに精度が落ちてきて困ったことになるだろう。
    ところが Haskell には分数のためのデータ型がある。その名は Rational で、Data.Ratio モジュールに入っている。
    Rational 型の値を作るには、分数のように書くだけである。分子と分母は % という記号で区切る。例えば、以下のような感じ。
    ...
    最初の例は四分の一 (1/4) である。2 例目では、半分を 2 つ足して 1 を作っている。
    3 例目は 1/3 + 5/4 = 19/12 の足し算。
    もう浮動小数なんて使うのはやめて、確率は Rational で表すことにしよう。

        [(3, 1 % 2), (5, 1 % 4), (9, 1 % 4)]

    OK、これで 3 が出る確率は 1/2 で、5 と 9 が出る確率はそれぞれ 1/4 である、ということがきれいに表現できた。

    さて、これはリストに対してさらに文脈を付加したものなので、こいつもきっと何らかの文脈付き値を表現しているのだろう。
    でも先に進む前に、これを newtype で包んでおこう。
    そうすると、これを何かのインスタンスにするときに役に立つ。
-}

newtype Prob a = Prob {getProb :: [(a, Rational)]} deriving Show

{-
    これはファンクターだろうか？　Yes。リストはファンクターだから、リストに何かを足したものである Prob もたぶんファンクターだろう。
    関数でリストを写すときは、リストの各要素に関数を適用したのだった。ここでも、各要素に関数を適用するのだが、確率はそのままにしておこう。
    では、ファンクターのインスタンスにしてみよう:
-}

instance Functor Prob where
    fmap f (Prob xs) = Prob $ map (\(x, p) -> (f x, p)) xs
    -- パターンマッチを使って newtype から中身を取り出し、関数 f を値に適用した上で、確率はそのままに、再度 Prob で包む。ちゃんと動くだろうか？

fmapTest :: Prob Int
fmapTest = fmap negate (Prob [(3, 1 % 2), (5, 1 % 4), (9, 1 % 4)]) -- Prob {getProb = [(-3,1 % 2),(-5,1 % 4),(-9,1 % 4)]}

{-
    OK。正しく動いている。

    では、お待ちかねの大問題。これってモナドだろうか？
    リストがモナドだったのだから、これもモナドであって欲しい気がする。
    🐶
    まず、return について考えよう。リストの return はどうだっただろうか？値を取って、単一要素リストに入れる関数だった。
    では Prob なら？　return はデフォルトの、最小限の文脈を持ってこないといけないのだから、きっと　Prob の return も単一要素リストを作るのだろう。
    確率のほうはどうしよう？ return x は、常に x を結果として提示するモナド値を作る関数だったから、確率が 0 だったりしたら変！
    常にその値を示す、というのなら 1 に決まっている。
    （そりゃそうで、リストモナドを考えても、return 5 :: [Int] とすれば [5] が出てくるのだから、非決定性計算でただし要素が 1 つしかないのなら確率 100 % に決まってる）

    >>= のほうは？　これはちょっと難しい。
    ここで、m >>= f （モナド値 m を、普通の値を取ってモナド値を返す関数 f に食わせる）は常に join (fmap f m) と等価であることを使い、
    どうすれば確率リストの確率リストを平らにできるか考えよう（join はネストされたモナドを平らにする関数）。
    例えば、 'a' か 'b' のいずれかが起こる確率がぴったり 25 % であるとしよう。
    そして 'a' と 'b' は同様に確からしいとする。
    さらに 'c' か 'd' のいずれかが起こる確率はきっかり 75 % であるとしよう。
    そして 'c' と 'd' もまた同様に確からしいとする。
        ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━┓　 ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
        ┃ 1/4    　　　　　   　　　　 ┃   ┃ 3/4    　　　　　   　　　　 ┃
        ┃ ┏━━━━━━━━━┓  ┏━━━━━━━━━┓  ┃   ┃ ┏━━━━━━━━━┓  ┏━━━━━━━━━┓  ┃
        ┃ ┃1/2      ┃  ┃1/2      ┃  ┃   ┃ ┃1/2      ┃  ┃1/2      ┃  ┃
        ...
        ┃ ┗━━━━━━━━━┛  ┗━━━━━━━━━┛  ┃   ┃ ┗━━━━━━━━━┛  ┗━━━━━━━━━┛  ┃
        ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━┛   ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
    では、これらの文字が出る確率は、それぞれいくらだろう？　これらの自称を 4 つの長方形で表したとしたら、それぞれの確率はいくらになるだろう？
    少し考えればわかるとおり、'a' が出る確率が 1/8、'b' も 1/8、'c' は 3/8、'd' も 3/8。

    この状況を確率リストで表すと、以下のようになる:
-}
...
{-
    こいつの型は Prob (Prob Char) である。では、入れ子になった確率リストを平らにする方法がわかった今、あとはどうやってコードに落とし込むかの問題である。
    そうすれば >>= は単に join (fmap f m) と書けて、モナドの一丁上がりとなる。
    この関数を flatten と名付けよう（join という関数はもう使われているから）。
    実装を以下に示す:
-}
...
{-
    関数 multAll は、確率リストとある確率 p のタプルを取って、リストの中のすべての確率を p 倍して、事象と確率の組のリストを返す関数。
    flatten は、multAll を入れ子確率リストの各要素に適用してまわり、得られた入れ子リストを最後に（リストとして）平らにする。
    これで必要なものはすべて揃った。今こそ Monad インスタンスを書くときである！
-}

-- Monad インスタンスにするには Applicative インスタンスになっている必要があるので、まず Applicative 型クラスのインスタンスにする。
...
instance Monad Prob where
    ...
    -- fail _ = Prob [] // fail はいらないっぽい（？）。‘fail’ is not a (visible) method of class ‘Monad’ というエラーが出る。

apProb :: Prob (a -> b) -> Prob a -> Prob b -- テキストにはこれは書いてないが、ap を Monad ベースで書いて、<*> は ap と同じだよ、とやればいい。
apProb mf m = do
    f <- mf
    x <- m
    return (f x)
...
{-
    😹🔍（テキストにはない。独自考察）
    ちなみに、<*> （すなわち apProb） の挙動がどうなるかは以下のとおり。
    *Main> Prob [((++ "!"), 1 % 1)] <*> Prob [("ha", 1 % 2), ("ho", 1 % 2)]
    Prob {getProb = [("ha!",1 % 2),("ho!",1 % 2)]}
    *Main> Prob [((++ "!"), 1 % 2), ((++ "?"), 1 % 2)] <*> Prob [("ha", 1 % 2), ("ho", 1 % 2)]
    Prob {getProb = [("ha!",1 % 4),("ho!",1 % 4),("ha?",1 % 4),("ho?",1 % 4)]}
-}

{-
    以上でコンパイルが透ようになるので、モナドのインスタンスにすることができた。
    できたばかりのモナドが、きちんとモナド則を満たしているかどうか試すこともとても重要。

    1️⃣モナド第一法則は、「return x >>= f と f x は等価であるべし」というものだった。
    　厳密な照明は省くが、まずある値を return でデフォルト文脈に入れ、ある関数を fmap し、それから結果の確率リストを平らにしたとしても
    （m >>= f は join (fmap f m) なので（ここでは m は return x）。ここでいう join は flatten 関数のことだけど）、 
    　関数が生み出すすべての確率に return が作った 1 % 1 が掛かるだけなので、文脈に影響はないことがわかる。

        ためしに（独自）：
            *Main> (return "nya") >>= (\x -> Prob [(x ++ "!", 1 % 2), (x ++ "?", 1 % 2)])
            Prob {getProb = [("nya!",1 % 2),("nya?",1 % 2)]}
            *Main> (\x -> Prob [(x ++ "!", 1 % 2), (x ++ "?", 1 % 2)]) "nya"
            Prob {getProb = [("nya!",1 % 2),("nya?",1 % 2)]}

    2️⃣モナド第二法則は、m >>= return は m と等価であると言っている。この例の場合、第一法則の場合のような考察をすれば、m >>= return が m と等しいことは明らか。
    　（return は確率 1 でその値を返すものだから）

        ためしに（独自）：
            *Main> Prob [('a', 1 % 2), ('b', 1 % 2)] >>= return
            Prob {getProb = [('a',1 % 2),('b',1 % 2)]}

    3️⃣モナド第三法則は、 f <=< (g <=< h) と (f <=< g) <=< h が等価であれ、と言っている。これもまた確かに成り立つ。
    　 なぜなら、確率モナドの基礎となるリストモナドは第三法則を満たしており、かつ、掛け算は結合法則を満たすからである。
    　　(1 % 2) * (1 % 3 * 1 % 5) は (1 % 2 * 1 % 3) * 1 % 5 と等しいよね。

    　　ためしに（独自）：以下。
-}

f1 :: Num a => a -> Prob a
f1 x = Prob [(x, 1 % 2), (-x, 1 % 2)]

f2 :: Num a => a -> Prob a
f2 x = Prob [(x*3, 1 % 3), (x*2, 2 % 3)]

f3 :: Num a => a -> Prob a
f3 x = Prob [(x+10, 3 % 4), (x-10, 1 % 4)]

thirdRuleTest :: Prob Int
thirdRuleTest = (f1 <=< (f2 <=< f3)) 30 -- Prob {getProb = [(120,1 % 8),(-120,1 % 8),(80,1 % 4),(-80,1 % 4),(60,1 % 24),(-60,1 % 24),(40,1 % 12),(-40,1 % 12)]}

thirdRuleTest' :: Prob Int
thirdRuleTest' = ((f1 <=< f2) <=< f3) 30 -- Prob {getProb = [(120,1 % 8),(-120,1 % 8),(80,1 % 4),(-80,1 % 4),(60,1 % 24),(-60,1 % 24),(40,1 % 12),(-40,1 % 12)]}

{-
    こうしてモナドが手に入ったからには何ができるだろうか？　もちろん、確率的計算ができるようになったのである。
    確率的な事象を文脈付きの値として扱い、すべての要素の確率を反映して、確率モナドが最終結果の確率をきちんと計算してくれるのである。

    さて、普通のコインが 2 枚と、10 回投げると 9 回裏が出るよう細工されたコインが 1 枚あるとしよう。これらのコインを全部同時に投げて、全部裏が出る確率はいくらだろうか。
    まず、普通のコイン投げとイカサマコイン投げに対応する確率値を作ってみよう。
-}
...
{-
    こいつを動かすと、イカサマコインが紛れ込んでいるとはいえ 3 枚とも裏が出る確率というのがいうほど大きくないことがわかる。
-}

allTails :: [(Bool, Rational)]
allTails = filter (\(isAllTails, _) -> isAllTails == True) (getProb flipThree) -- [(True,9 % 40)]

{-
    3 枚とも裏が出る確率は 9 / 40 である。これは 25 パーセントよりも少ない。
    また、3 枚のうちのどれかが表になり、結果が False になるパターンは 7 通りあるのだが、どうやらこのモナドはこれらを統合する方法は知らないようである。
    といっても、それを実装するのは簡単。

    さて、この節では、「リストが確率の情報も持っていたらどうなるだろう？」という疑問から出発して、型を作り、モナドを見切り、モナドのインスタンスを作って、
    何がしかの計算ができるようになった🎉
-}
