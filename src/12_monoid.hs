...
-------------------------------
{-
    いろんなデータ構造の上に畳み込みを定義したい……、そういうときにもモノイドが活躍する。
    これまでは、リストの畳み込みしか紹介しなかった。
    しかし、畳み込みできるデータ構造はリストだけではない。むしろ、ほとんどすべてのデータ構造の上に畳み込みを定義できるのである。
    木構造などは畳み込みしやすいデータ構造の典型。

        ❌　仕様が変わったので、Data.Foldable をインポートする必要はない。
        × 畳み込みと相性の良いデータ構造は実にたくさんあるので、Foldable 型クラスが導入された。
        × Functor が関数で写せるものを表すように、Foldable は畳み込みできるものを表している。

        × Foldable 型自体は標準で組み込まれているようだが、それにまつわる関数群は Data.Foldable にある。
        × Data.Folable が提供する関数群の名前は Prelude の関数と衝突するので、Data.Foldable モジュールは修飾付きインポートするのがおすすめ。

        ×     import qualified Data.Foldable as F

        × Data.Foldable が提供する関数には foldr、foldl、foldl1、……などがある。でも、これらは Prelude にもあった（リスト操作）。
        × 比較してみる。
        ...
        💡昔は foldr :: (a -> b -> b) -> b -> [a] -> b だったが、今は上を見てのとおり Prelude の foldr も
        　 Data.Foldable.foldr もどちらも Foldable 型クラス制約がついているので、同じ。Data.Foldable を import する必要はない。

    畳み込みをサポートするデータ構造はリスト以外にもある。例えば、Maybe もそうである。
-}

folded1 :: Int
folded1 = foldl (+) 2 (Just 9) -- 11

folded2 :: Bool
folded2 = foldr (||) False (Just True) -- True

{-
    しかし、Maybe を畳み込むのはそこまで面白いものではない。
    Maybe は単に、Just 値だったら 1 要素のリストのように振る舞い、Nothing 値なら空リストのように振る舞うだけである。
    もう少し複雑なデータ構造を試してみよう。

    第 7 章で木構造を作ったが、そのときの定義は次のような感じだった:
-}

data Tree a = EmptyTree | Node a (Tree a) (Tree a) deriving (Show)

{-
    木は、値を持たない空の木か、1 つの値と 2 つの木を持つノードからなる再帰的構造として定義した。
    木を定義したあと、木を Functor のインスタンスにすることで、木に関数を fmap できるようになった。
    今度は、木を Foldable のインスタンスにして木に畳み込みを使えるようにしよう。

    ある型コンストラクタを Foldable のインスタンスにする 1 つの方法は、直接 foldr を実装することである。
    だが、通常はもっと簡単な方法がある。それは foldMap 関数を実装することである。
    foldMap 関数は次のような型を持っている。

        foldMap :: (Foldable t, Monoid m) => (a -> m) -> t a -> m

    foldMap 関数の第一引数は、「Foldable にしたいコンテナの中身（a）の型の値を取って、モノイドを返す」関数である。
    foldMap 関数の第二引数は、a 型の値を含む Foldable 構造自身である。
    foldMap 関数は、まず第二引数の構造体に第一引数の関数を map して、構造体の中身をごっそりモノイド値に変える。
    次にそれらモノイド値を mappend して単一のモノイド値へと結合する。
    この動作は何だか不自然に思えるかもしれないが、すぐに、これは実に簡単に実装できる仕様なのだとわかる。
    そしてこの foldMap さえ定義すれば、ある型を Foldable にできる。
    つまり、ある型の foldMap さえ定義すれば、その型の foldr や foldl は自動的に手に入るのである。

    Tree を Foldable のインスタンスにする方法は次のとおり:
-}
...
{-
    〜解説〜
    さて、我々の木構造の要素を取ってモノイド値を返す関数を誰かがくれたとする。
    その関数を使って、木構造全体を 1 つのモノイド値に帰着するにはどうすればいいだろう？
    fmap のときは、与えられた関数をその場でノードに適用し、さらに左右 2 つある部分木にも再帰的に fmap を適用した。
    今度は、関数を map するだけではなく、その結果を mappend して単一のモノイド値にする作業も必要である。

    まず空の木の場合、つまり値もなければ部分木もないという場合には、モノイド作りの材料が何もないので mempty になるしかない。
    空じゃない木（Node）の場合はもうちょっと面白くなる。
    こちらは 2 つの部分木と 1 つの値を含む。この場合は、まず左右の部分木に再帰的に f を foldMap しよう。
    それぞれ単一のモノイド値が返ってくるはずである（foldMap はそういうものだから）。
    それからノードが持っている値にも f を適用する。
    これで 3 つのモノイド値（2 つは部分木からきたもので、1 つはノード内の値に f を適用してできたもの）が揃った。
    あとはこの 3 つを 1 つの値に畳むだけ。ここは mappend の出番である。

    foldMap の実装自体は、値をモノイドに変換する関数を必要としなかったことに注目！
    変換関数は foldMap に引数として与えられるので、foldMap の実装に必要なのは、変換関数は所与のものとして、どこに適用して結果のモノイドをどう結合するのかを決めてやることなのである。
    こうして我々の木は Foldable インスタンスになった。これで foldr と foldl も無料で手に入ったわけである。
-}
...
foldedValue :: Int
foldedValue = foldl (+) 0 testTree -- 42

foldedValue' :: Int
foldedValue' = foldl (*) 1 testTree -- 64800

{-
    foldMap は Foldable のインスタンスを定義するためだけの存在ではない。
    Foldable な構造を単一のモノイド値に畳みたいときには実際役に立つ。
    例えば、この木の中に 3 に等しい数があるか調べたかったら、次のように書けばよい:
-}

has3 :: Bool
has3 = getAny $ foldMap (\x -> Any $ x == 3) testTree -- foldMap をすると、Tree の各要素について、それが 3 と等しいかどうかの True/False を Any モノイドにしたものたちを
                                                      -- mappend (<>) して単一のモノイド値（`Any True` あるいは `Any False` にしてくれる）

{-
    ここで \x => Any $ x == 3 は、「数を取って `Any にくるまれた Bool というモノイド値` を返す関数」である。
    foldMap はこの関数を木のすべての要素んい適用してから、mappend (<>) を使って 1 つのモノイドに畳んでくれるわけである。
    例えば以下のようなことをすると
-}

hasOver15 :: Bool
hasOver15 = getAny $ foldMap (\x -> Any $ x > 15) testTree -- False

{-
    木の中のすべてのノードの値は、ラムダ式の中の関数を適用された結果 Any False に変わる。
    複数の Any を mappend して True が変えるのは、そのうち 1 つでも True が含まれる場合に限る。
    testTree には 15 より大きい要素は含まれていないため、False が返ってくるのは当然の結果。
-}

{-
    木構造をいともたやすくリストに変換できる技もある。
    \x -> [x] を foldMap するのである。このラムダ織は、木の各要素を単一要素のリストに変換する。
    それから mappend がこれらリストモノイドの間に適用され、元の木の要素をすべて含んだリストという単一のモノイドが出来上がる。
    しかもこれらの技は、木構造に限らず、あらゆる Foldable なデータ構造に適用できる！
-}

listFromTree :: [Int]
listFromTree = foldMap (\x -> [x]) testTree -- [1,3,6,5,8,9,10]
