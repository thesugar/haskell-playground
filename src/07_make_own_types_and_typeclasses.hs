...
-------------------------------
{-
    Functor は、全体を写せる（map over）ものの型クラスである。
    ここでいう「写す」とは、「そのままコピーを取る」写真のイメージではなくて、「何か変換を施す」写像のイメージである。
    "map over" と聞いて、Haskell の頻出イディオムである「リストの map」を思い出そう。
    あれも何かを写す操作の典型である。
    だから、リストは Functor 型クラスに属している。

    Functor 型クラスを知りたいと思ったら実装を見るのが一番。
        class Functor f where
            fmap :: (a -> b) -> f a -> f b

    見ての通り、Functor は 1 つの関数 fmap を持っており、デフォルト実装は提供していない。
    fmap は面白い型を持っている。これまでに出てきた型クラスでは、インスタンスになった型を表す型変数は、
    (==) :: Eq a => a -> a -> Bool
    のようにすべて具体型だった。しかし今度の f は、1 つの型引数を取る型コンストラクタである。
    （軽く復讐：Maybe Int は具体型で、Maybe は 1 つの型を引数に取る型コンストラクタである）

    どうやら fmap は、「ある型 a から別の型 b への関数」と「ある型 a に適用されたファンクター値」を取り、
    「別の型 b のほうに適用されたファンクター値」を返す関数のようだ。

    ところで、fmap の型宣言は何かに似ていると思わないだろうか？
    map 関数の型シグネチャを思い出そう。
        map :: (a -> b) -> [a] -> [b]

    面白い！ map は「ある型から別の型への関数」と「ある型のリスト」を取り、「別の型のリスト」を返す関数のようだ。
    どうやらファンクターの正体が見えてきた。実は、map というのはリスト限定で動作する fmap に過ぎない。
    リストに対する Functor インスタンス宣言は次のようになる：
        instance Functor [] where
            fmap = map
    以上！（なお、ここで Functor のインスタンスとして宣言されている [] は空リストのことではなく、
    リスト型の型コンストラクタとしての [] である。型名 [a] は [] a とも書けることを思い出そう）
    instance Functor [a] where と書いていないことに注目。これは、f は 1 つの型を取る型コンストラクタだからである。
    以下の宣言を見ても明らかだろう。
        fmap :: (a -> b) -> f a -> f b
    [a] は任意の型を要素とするリストを表す具体型になっているのに対し、[] はまだ具体型ではなく、1 つの型引数を取って
    [Int], [String], [[String]] といった型を生み出す型コンストラクタである。
    リストにとっての fmap はただの map であるため、2 つの関数をリストに使った結果は一致する。

        fmap (*2) [1..3] -- [2,4,6]
        map (*2) [1..3] -- [2,4,6]

    map や fmap を空リストに対して使ったら何が起こるだろう？
    もちろん、空リストが返ってくる。これらは、[a] 型の空リストを [b] 型の空リストに変える働きをする。
-}

--- ¶ Maybe は Functor だよ、たぶん
{-
    ファンクターになれるのは、箱のような働きをする型である。リストというのは、空だったり、物がいくつか入っていたりする箱だとみなせる。
    リストの中身にまた箱が入っていて、その中身も空だったり、さらに別のものが入っていたりするかもしれない。
    では、ほかにも箱のようなものはあるだろうか？
    例えば maybe a 型がそうだ。そう思ってみると、Maybe とは、何も入っていなければ Nothing 値になり、
    何か、例えば "haha" が入っていれば Just "haha" という値になる箱のように見えないだろうか？

    Maybe は以下のようにファンクターになっている。

        instance Functor Maybe where
            fmap f (Just x) = Just (f x)
            fmap f Nothing = Nothing

    ここでも instance Functor Maybe where と書いており、instance Functor (Maybe m) where とは書いていないことに注意。
    Functor は具体型ではなく、型コンストラクタを要求しているからである。
...
--- ¶ Tree も Functor の森に
{-
    「関数で写せるもの」であるため、Functor に属する型はほかにもある。
    Tree a 型がそうである。木は、複数の値を持たせることも空にすることもできる箱のようなものだし、
    Tree 型コンストラクタは引数を 1 つだけ取るからである。
    fmap を Tree 専用の関数を思って眺めた場合、その型シグネチャは
        fmap :: (a -> b) -> Tree a -> Tree b
    となるだろう。

    さて、ここは再帰の使いどころである。ある関数で空の木を写すと、空の木が生じる。
    一方、空でない木を写すときは、ルート値はその関数を適用したもの、それから左右の部分木は元の部分木が写されたものになるはずだ。
    以下がそのコードである。
-}
...
{-
    試してみよう。
        ...
        *Main> fmap (*2) EmptyTree 
        EmptyTree
        *Main> fmap (*4) (foldr treeInsert EmptyTree [3,7,5])
        Node 12 EmptyTree (Node 28 (Node 20 EmptyTree EmptyTree) EmptyTree)

    でも、Tree a 型を二分探索木を実装するのに使っている場合は気を付けること。
    二分探索木を任意の関数で写したあとの木も二分探索木のままである保証はない。ある木が二分探索木とみなせるためには、
    各ノードについて、その左部分木の要素はすべてそのノードの要素より小さく、右部分木の要素は大きくないといけない。
    でも、例えば negate のような関数で二分探索木を写すと、左部分木の要素はたちまちルートの要素より大きくなってしまい、二分探索木だったものはただの二分木になってしまう。
-}

--- ¶　Either は Functor であるか否か
{-
    では、Either a b はどうだろう？　これはファンクターにできるだろうか？
    Functor 型クラスは、型引数を 1 つだけ取る型コンストラクタを要求しているが、Either は引数を 2 つ取る。
    では、Either に引数を 1 つだけ部分適用して、自由変数を 1 つ残した状態にすればいいんじゃなかろうか？
    Either a はファンクターとして次のように定義されている。

    instance Functor (Either a) where
        fmap f (Right x) = Right (f x)
        fmap f (Left x) = Left 

    ただの Either ではなく Either a がインスタンスにされていることがよくわかる。
    fmap がもし Either a に特化していたら、その型シグネチャは次のようになっていただろう:
        fmap :: (b -> c) -> Either a b -> Either a c

    Right 値コンストラクタがきた場合には関数が適用されるが、Left 値の場合には何もしない。なぜか？
    Either a b 型の定義に戻れば理由は見て取れる。

        data Either a b = Left a | Right b

    もし、左と右の両方を同じ関数で写したかったら、a と b は当然同じ型である必要がある。
    考えてもみよ。仮に文字列を取って文字列を返す関数で Either a b を写せと言われたとして、b は文字列型だけど a は数値型だったら、もう詰んでいるよね。
    それに、fmap が Either a b に作用したときの型を見ても、2 つ目の引数は変化してもいいけど 1 つ目の引数は不変でないといけないことがわかる。
    そして、1 つ目の引数は Left 値コンストラクタが使っていることもわかる。
    （【再掲】　fmap :: (b -> c) -> Either a b -> Either a c）

    これは箱の比喩ともうまく合致する。Right は中身の入った箱、Left は空の箱のようなもので、どうして空になってしまったのかを表すエラ〜メッセージが箱の側面に書かれているのである。
    Data,Map の関数もファンクター値にできる。Map もまた箱の入っている（入っていないかも！）箱とみなせるからである。
    Map k v の場合、fmap は v -> v' という関数で、Map k v を写して Map k v' という Map を返す。
-}
...
-------------------------------
{-
    型コンストラクタは、他の型を引数に取っていずれは具体型になる。
    この振る舞いは、関数が値を引数に取って値を生み出すのとよく似ている。
    それに、関数と同じく型コンストラクタも部分適用できる。
    例えば、Either String は型引数をあと 1 つ取って具体型、例えば Either String Int を生み出す型コンストラクタである。

    3, "YEAH", そして takeWhile といった値（そう、関数も受け取ったり渡したりできる値である。Haskell では）は、それぞれ固有の型を持っている。
    型とは、値について何らかの推論をするために付いている小さなラベルである。
    そして、型にも小さなラベルが付いているのである。それは種類（kind）。
    種類は、「型の型」のようなもの。

    さっそく GHCi で肩の種類を調べてみよう。
        *Main> :k Int
        Int :: *
    この * とはなんだろう？　これは、具体型を表す記号である。
    具体型とは型引数を取らない型のこと。値に付けられる型は具体型だけである。
    ...
    Maybe は 1 つの具体型（例えば Int）を取って具体型（例えば Maybe Int）を返す型コンストラクタであることがわかる。
    ちょうど、Int -> Int といえば Int を取って Int を返す関数を指すように、* -> * は 1 つの具体型を取って 1 つの具体型を返す関数を意味するのである。
    Maybe に型引数を与えるとどんな種類の型ができるか調べてみよう。

        *Main> :k Maybe Int
        Maybe Int :: *

    予想通り、Maybe に型引数を与えると具体型になっていた。

    さて、では Either の種類を見てみよう。

        *Main> :k Either 
        Either :: * -> * -> *

    Either は 2 つの具体型を取って具体型を返す関数である、と行っている。
    これもまた 2 つの引数を取って何かを返す関数の型宣言とどこか似ている。
    型コンストラクタは（関数と同様）カリー化されているので、部分適用できる。
    ...
    Either を Functor 型クラスのインスタンスにしようとしたときは、部分適用をして Either a の形にする必要があった。
    Functor は型引数を 1 つ取る型を要求にしているのに対し、Either は型引数を 2 つ取るからである。
    言い換えると、Functor になれる型は種類が * -> * の型なので、Either を部分適用して元の種類 * -> * -> * から * -> * という種類に変換する必要があったのだ。

    改めて Functor の定義を見直すと、型変数 f は 1 つの具体型を取って具体型を生み出す型として扱われていることが見て取れる。

        class Functor f where
            fmap :: (a -> b) -> f a -> f b

    関数の型宣言で値の型として使われていることから、f a や　f b は具体型でないといけないということがわかる。
    そこから、FUnctor と友達になりたい型は * -> * という種類を持つ必要があることがわかる。
-}