...
{-
    ファンクターやアプリカティブファンクターと同様、モナドにも、すべてのモナドインスタンスが守るべき法則がいくつかある。
    ある型が Monad のインスタンスになっているからといって、実際にモナドであるわけではない。
    ある型が真にモナドであるためには、その型はモナド則を満たさねばならない。
    モナド則があることで、モナドになっている型や、型の振る舞いについて、合理的な想定ができるようになる。

    Haskell では、型検査が通る限り、任意の型を任意の型クラスのインスタンスにできてしまう。
    ある型がモナド則を満たしているかどうかなどの検査はしてくれないので、Monad 型クラスのインスタンスを自作する場合には、その型がモナド則を満たしていることを自分で保証しないといけない。
    標準ライブラリに含まれる型はモナド則を満たしていると信頼できるが、自分でモナドを作るときには、モナド則が成り立っているか手動で確認しないといけない。
-}

--- ¶　左恒等性
{-
    第一のモナド則が言っているのは、return を使って値をデフォルトの文脈に入れたものを >>= を使って関数に食わせた結果は、
    単にその値にその関数を適用した結果と等しくなりなさい、ということである。
    形式的に言えば、return x >>= f と f x は等価である、ということ。
    ...
    モナド値を文脈付きの値とみなし、return は値をその値が再現できるような最小限のデフォルトの文脈に入れるものだとみなす立場からすれば、
    このモナド則は実に自然な発想だろう。
    return が作る文脈が本当に最小限のものだというのなら、return で作ったモナド値を関数に食わせるという操作は、単に関数を普通の値に適用する操作と
    ほんの少ししか食い違わないだろう。そして実際、食い違いはゼロなのである。

    Maybe モナドに関しては、return は Just に等しいと定義されている。
    Maybe モナドとは、そもそも失敗する可能性のある計算を表現するモナドだったから、そのような文脈に値を入れたいというのなら、
    その値を計算の成功として扱うのは自然なことだ。返すべき値がわかっているのだから。
    以下は Maybe モナドの return を使っている例。
-}

左恒等性の例 :: Maybe Int
左恒等性の例 = return 3 >>= (\x -> Just (x + 100000)) -- Just 100003

左恒等性の例' :: Maybe Int
左恒等性の例' = (\x -> Just (x + 100000)) 3 -- Just 100003

{-
    リストモナドの場合、return は引数を単一要素リストに入れる関数である。そして >>= の実装は、
    リストの中のすべての要素に対して関数を適用するというものだった。
    ただ、単一要素リストには要素が 1 つしかないから、関数を値に直接適用したのと同じ結果になるのである。
-}

左恒等性の例_リスト :: [String]
左恒等性の例_リスト = return "WoM" >>= (\x -> [x,x,x]) -- ["WoM","WoM","WoM"]

左恒等性の例_リスト' :: [String]
左恒等性の例_リスト' = (\x -> [x,x,x]) "WoM" -- ["WoM","WoM","WoM"]

--- ¶ 右恒等性
{-
    モナドの第二法則は、>>= を使ってモナド値を return に食わせた結果は、元のモナド値と不変であると言っている。
    式で書くと、 m >>= return はただの m である、ということ。

    この法則は、第一法則ほど自明ではないかもしれない。
    なぜこれが成り立つべきなのか見ていこう。
    >>= を使ってモナド値を関数に食わせるという式があった場合、関数のほうは、普通の値を取ってモナド値を返す関数のはずである。
    そして return も、型を見れば、そういう関数の一種であることがわかる（return :: Monad m => a -> m a）。

    return は、値を “その値を返せるような最小限の文脈” に入れるもの。
    これは、例えば Maybe モナドであれば決して失敗しないことを意味し、リストモナドであれば非決定性を増やさないことを意味する。
    何種類かのモナドで試してみよう。
-}

右恒等性の例 :: Maybe String
右恒等性の例 = Just "move on up" >>= return -- Just "move on up"

右恒等性の例' :: [Int]
右恒等性の例' = [1,2,3,4] >>= return -- [1,2,3,4]

右恒等性の例'' :: IO ()
右恒等性の例'' = putStrLn "Wah!" >>= return -- Wah!

{-
    リストの場合、>>= の実装は以下のようになっていた。
        xs >>= f = concat (map f xs)

    ということは、[1,2,3,4] を return に注ぎ込むと、まず return が [1,2,3,4] を写して [[1], [2], [3], [4]] ができる。
    （map return [1,2,3,4] :: [[Int]] とターミナルに打ち込むと結果が [[1], [2], [3], [4]] となるのが確認できる）
    それからこのリストが concat されて、元のリストに戻る。

    左恒等性と右恒等性は、いずれも基本的に return の振る舞いに関する法則。
    return はモナドシステムの中で、通常の値をモナド値に変えるという重要な役割を担っており、
    return が作り出すモナド値が最小限でない余計な文脈を持っていたら困ってしまう。
-}

--- ¶　結合法則
{-
    最後のモナド則は、>>= を使ったモナド関数適用の連鎖があるときに、どの順序で評価しても結果は同じだというもの。
    式で書くと、(m >>= f) >>= g と m >>= (\x -> f x >>= g) が等価である、ということ。
    ・・・🤔❓

    m はモナド値で、f と g はモナド関数。
    (m >>= f) >>= g と書くと、m を f に食わせることになる。その結果はモナド値であり、そのモナド値を g に食わせる。
    一方、m >>= (\x -> f x >>= g) と書いたときは、x を取るラムダ式があって、それはモナド値 f x を g に食わせたものを返す。
    以上の 2 つが等価であるというのは、わかるようでわかりにくいので、例を見て理解しよう。

    バランス棒の問題を覚えているだろうか？
    バランス棒に鳥がとまりに来る様子をシミュレートするために、失敗するかもしれない関数の連鎖を作ったんだった。
-}

polepole :: Maybe Pole
polepole = return (0, 0) >>= landRight' 2 >>= landLeft' 2 >>= landRight' 2 -- Just (2,4)

{-
    この例では、まず Just (0,0) があって、それにモナド関数 landRight' 2 を適用している。
    その結果はまた別のモナド（Just (0, 2)）であり、それがまた次のモナド関数に束縛され、と続いている。
    この式にあえて括弧をつければ、次のようになる。
-}
...
{-
    しかし、このルーチンは次のように書くこともできるのである。
-}

polepole'' :: Maybe Pole
polepole'' = return (0, 0) >>= (\x ->
            landRight' 2 x >>= (\y ->
            landLeft' 2 y >>= (\z -> 
            landRight' 2 z))) -- Just (2,4)

{-
    return (0,0) は Just (0,0) と等価であり、これを >>= でラムダ式に食わせると x が (0,0) になる。
    landRight' は鳥の数とバランス棒の状態（数のタプル）を取る関数だが、それらを無事受け取って Just (0,2) を返す。
    それを次のラムダ式に >>= により渡すと、y が Just (0, 2) になる。
    このような計算が、最後の鳥が処理されて Just (2, 4) が生み出されるまで続く。
    これが全体の計算結果になるわけである。

    このようにモナド値をモナド関数に食わせる式を評価するとき、入れ子の順序はどうでもよく、ただ関数の意味だけが重要である、
    というのがモナド結合法則の言うところである。

    この法則を別の側面から見てみよう。いま、f と g という名の 2 つの関数があるとする。

        (.) :: (b -> c) -> (a -> b) -> (a -> c)
        f . g = (\x -> f (g x))

    g の型が a -> b で、f の型が b -> c ならば、2 つの関数を結合し、g の返り値を f の引数に渡すことで、
    a -> c 型の新しい関数を作れる。
    では、この 2 つがモナド関数であったらどうだろう？　2 つの関数の返り値がモナド値であったら？
    a -> m b 型の関数の返り値は、そのままでは b -> m c 型の関数に渡せない。
    後者は b 型の通常の値を期待しており、モナド値は受け取れないからである。ところが、>>= を使うとこれが可能になる。

    (<=<) :: (Monad m) => (b -> m c) -> (a -> m b) -> (a -> m c)
    f <=< g = (\x -> g x >= f)

    こうして、モナド関数を合成することが可能になった！
-}
...
{-
    これは素晴らしい！　でも、これと結合法則にどんな関係が？
    モナド結合法則をこの関数合成の法則として見た場合、f <=< (g <=< h) と (f <=< g) <=< h が等価である、という宣言になっている。
    これもまた、モナドの演算子を入れ子にする順番はどうでもいい、ということの別の表現になっているのである。

    ここで、<=< を使って最初の 2 つの法則を書き直してみると、左恒等性は任意のモナド関数 f に対して
    f <=< return は f と等価であるという宣言、右恒等性は return <=< f もまた f と等価であるという宣言になっている。
    （左恒等性：return x >>= f と f x は等価である）
    　右恒等性：m >>= return はただの m である）

    こうしてみると、3 つのモナド則は、f が通常の関数であった場合に (f . g) . h と f . (g. h) が等価であり、
    f . id はつねに f と等しく、id . f もただの f であることと非常に似ている。

    この章では、モナドの基礎を見て、Maybe モナドとリストモナドの仕組みを学んだ。
    次の章では、ほかにもたくさんあるすごいモナドを紹介していき、独自のモナドも作ってみる。
-}